...
func Init(levelStr string) {
	...
	// Create log filename with unix timestamp: streamnzb-{unix_timestamp}.log (one file per run)
	ts := time.Now().Unix()
	logFileName := fmt.Sprintf("streamnzb-%d.log", ts)
	logFilePath := filepath.Join(dataDir, logFileName)

	// Ensure data directory exists and open log file (only on first Init; SetLevel preserves it)
...
// SetLevel updates the logger level at runtime (preserves existing log file)
