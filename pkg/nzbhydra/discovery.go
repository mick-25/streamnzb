...
// GetConfiguredIndexers discovers all active Usenet indexers from NZBHydra2 by running
// a minimal search and extracting indexer names + hosts from hydraIndexerName and
// hydraIndexerHost attributes. One call gives us everything we need for indexers and
// AvailNZB pre-indexing. Only indexers that return results for the search query appear.
func GetConfiguredIndexers(baseURL, apiKey string, um *indexer.UsageManager) ([]indexer.Indexer, []string, error) {
	...
	for _, nh := range seen {
		clientName := fmt.Sprintf("NZBHydra2:%s", nh.name)
		idx, err := NewClientWithIndexer(baseURL, apiKey, clientName, nh.name, um)
		if err != nil {
			logger.Error("Failed to init NZBHydra2 indexer", "name", nh.name, "err", err)
			continue
		}
		indexers = append(indexers, idx)
		logger.Info("Initialized NZBHydra2 indexer", "name", nh.name)
	...
	if len(indexers) == 0 {
		return nil, nil, fmt.Errorf("could not initialize any NZBHydra2 indexers from search results")
	}
	...
	logger.Debug("NZBHydra2 indexers", "indexers", names, "hosts", hostnames, "count", len(indexers))
